...
from dataset_j import ProfileClassEqualSplitTrainMaskDataset, EvalMaskDataset
...
def train_and_eval(done_epochs: int, train_epochs: int, clear_log: bool = False) -> None:
    ...
    for epoch in range(done_epochs, done_epochs + train_epochs):
        ...
        for batch_index, (images, label) in enumerate(train_loader):
            ...
            total += labels.size(0)
            correct += (lam*prediction.eq(torch.argmax(labels_a, dim=1)).cpu().sum().float() 
                        + (1-lam)*prediction.eq(torch.argmax(labels_b, dim=1)).cpu().sum().float())
        ...
        with torch.no_grad():
            ...
            for batch_index, (images, label) in enumerate(val_loader):
